...
interface EmptyPhotoCardProps {
  message?: string;
  showIcon?: boolean;
}

export default function EmptyPhotoCard({
  message = "Photo not available right now",
  showIcon = true,
}: EmptyPhotoCardProps) {
  return (
    <div className="h-full rounded-2xl border border-white/10 bg-[#1B1B1B] p-4">
      <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-white/20 bg-[#252525] px-3 py-12 h-48">
        {showIcon && <ImageOff className="h-8 w-8 text-white/40 mb-3" />}
